...
今天做的工作是把后台规范化，整理出了信号的形式，然后又统一整合了一个Controller类，负责协调IP，Client，Server的各种信号，工作流程等等。然后基本调试完成了，包括文件和消息接收功能，发送功能也已经基本测试完成了，有一点小小的问题，应该很快能解决(包含发送测试代码的版本没有上传)

现有的问题是，针对多连接的连接信息存储方式可能有问题。

另外，没有使用多线程，因此文件的发送与接收会导致界面卡死。

2020.3.9 10：34

刚才花了两个小时试图解决多线程的问题，我使用的多线程是将三个主要的工作类都movtothread，也为此作了不少的改造，但是最终程序还是出了问题，主要面临的就是无法在一个线程向另一个线程增删socket，但是对于三个工作类来说，这些是常规操作。我卡死了。

刚才花了不少功夫将代码逐步恢复到没有多线程的样子，但是依旧不能保证百分百还原了，但是至少功能没啥问题。

我想我还是需要花不少时间，好好研究研究多线程在Qt里面怎么使用，如果能只把大文件的发送和接受部分放进线程就好了。



2020.3.9 23：13

多线程的问题实在是比我想象的要复杂的多，而Qt的多线程真的是用起来太复杂了。现在的版本，我终于搞定了，把三个工作类都加入了线程，然后也终于不再报关于线程的错误了。

这是我学到的东西：

1. 使用movetothread使用线程
